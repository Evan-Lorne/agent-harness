...
生产环境推荐 Python 3.11，搭配 uv 作为依赖和虚拟环境管理器。服务器建议 4 核 8GB 以上配置，SSD 硬盘。

数据库使用 PostgreSQL 16，需要预先创建数据库和用户。连接字符串格式：`postgresql://user:password@host:5432/dbname`。

## 部署步骤

第一步是拉取代码并安装依赖。使用 `git clone` 获取最新代码，然后运行 `uv sync --frozen --no-editable` 确保依赖版本一致。

第二步是配置环境变量。复制 `.env.example` 为 `.env`，填入数据库连接、API Key 等敏感信息。注意不要把 `.env` 提交到 Git。

第三步是运行数据库迁移。执行项目约定的 Python 迁移命令应用所有待执行的迁移脚本。如果迁移失败，检查数据库连接是否正常。

第四步是构建发布产物。运行 `uv build` 生成 wheel 和源码包。

第五步是启动服务。使用 `uv run --no-sync agent-harness` 启动，生产环境可交给 systemd 或容器编排平台管理。
...
如果部署后发现问题需要回滚：
1. 停止当前服务：使用 systemd 或容器编排平台停止当前实例
2. 切回上一个版本：`git checkout v1.2.3`
3. 重新安装依赖并构建：`uv sync --frozen --no-editable && uv build`
4. 回滚数据库（如有迁移）：执行项目约定的 Python 迁移回滚命令
5. 重启服务：使用 systemd 或容器编排平台启动上一版本实例
